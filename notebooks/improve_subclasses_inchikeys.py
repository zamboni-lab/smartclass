...
df_inchikeys = df_inchikeys.with_columns([pl.col("inchikey").str.slice(0, 14).alias("short_inchikey")])

# Perform a self-join on the short_inchikey column
df_joined = df_inchikeys.join(df_inchikeys, on="short_inchikey", how="inner", suffix="_right").drop("short_inchikey")
...
df_filtered = df_filtered.filter(pl.col("inchikey") != pl.col("inchikey_right")).drop("inchikey", "inchikey_right")

# Join the filtered result with the InChI DataFrame on the InChI key
df_filtered = df_filtered.join(df_inchis_filtered, left_on="structure_right", right_on="structure", how="inner")
